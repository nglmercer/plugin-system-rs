...
pub(crate) async fn send_hotkey(
    ...
    let sim_result = tokio::task::spawn_blocking(move || {
        ...
        crate::api::helpers::call_plugin_ok_sync(
            &guard,
            "key-simulator",
            "simulate_keys",
            serde_json::json!({"keys": keys_for_sim}),
        )
    })
    .await
...
pub(crate) async fn record_hotkey(
    ...
    let result = tokio::task::spawn_blocking(move || {
        let guard = pm.blocking_read();
        crate::api::helpers::call_plugin_raw_sync(
            &guard,
            "key-simulator",
            "listen_for_combo",
            serde_json::json!({"timeout_ms": req.timeout_ms}),
        )
...
pub(crate) async fn reset_hotkey_recording(
    ...
    let pm = state.plugin_manager.plugin_manager().clone();
    let guard = pm.blocking_read();
    let _ = crate::api::helpers::call_plugin_raw_sync(
        &guard,
        "key-simulator",
        "reset_recording",
        serde_json::json!({}),
