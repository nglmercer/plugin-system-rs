...
export function WidgetGrid() {
  ...
  useEffect(() => {
    fetchDashboard().then((data) => { setLayout(data); setLoading(false); }).catch(() => setLoading(false));
  }, []);

  function persist(next: DashboardLayout) { setLayout(next); saveDashboard(next); }
  ...
  function handleSaveWidget(id: string, updates: { title?: string; colSpan?: number; settings?: Record<string, any> }) {
    persist({ ...layout, widgets: layout.widgets.map((w) => w.id === id ? { ...w, ...updates } : w) });
  ...
  if (loading) return h("div", { class: "dashboard-loading" }, "Loading dashboard...");
  ...
  return h("div", { class: "dashboard-root" },
    ...
    layout.widgets.length === 0
      ...
      : h("div", { class: "dashboard-grid", style: { gridTemplateColumns: `repeat(${layout.columns}, 1fr)` } },
          layout.widgets.map((widget) =>
            h("div", { key: widget.id, class: `dashboard-widget variant-${widget.settings.variant || "compact"}`,
              ...
              h("div", { class: "widget-header" },
                ...
                h("div", { class: "widget-controls" },
                  h("button", { class: "widget-control-btn edit", onClick: () => setWizardWidget(widget.id) }, "E"),
                  h("button", { class: "widget-control-btn remove", onClick: () => handleRemoveWidget(widget.id) }, "X"),
                ),
              ),
              h("div", { class: "widget-content" }, h(WidgetContent, { widget })),
            ),
          ),
        ),
    showLibrary && h(WidgetLibrary, { onAdd: handleAddWidget, onClose: () => setShowLibrary(false) }),
    editing && h(WidgetWizard, {
      widget: editing,
