...
function WidgetWizard({
  ...
  const [variant, setVariant] = useState<string>(
    widget.settings.variant || getDefaultVariant(widget.type),
  );
    ...
    h(
      ...
      h(
        ...
        h(
          "div",
          { class: "wizard-title" },
          `Edit: ${WIDGET_CATALOG.find((w) => w.type === widget.type)?.label || widget.type}`,
        ),
        h("button", { class: "picker-close", onClick: onClose }, "X"),
      ),
...
function WidgetContent({ widget }: { widget: WidgetConfig }) {
  switch (widget.type) {
    ...
    case "quick-actions":
      return h(QuickActionsWidget, null);
    case "send-hotkey":
...
function SystemMonitorWidget({ settings }: { settings: Record<string, any> }) {
  ...
  const cpuColor =
    stats.cpu_usage < 50
      ...
      : stats.cpu_usage < 80
        ...
        : "#f44336";
  const memColor =
    stats.memory_usage < 60
      ? "#2196f3"
      : stats.memory_usage < 85
        ? "#ff9800"
        : "#f44336";
  ...
  if (variant === "minimal") {
    return h(
      "div",
      { class: "sysmon-variant minimal" },
      h(
        "div",
        { class: "sysmon-big", style: { color: cpuColor } },
        `${stats.cpu_usage.toFixed(0)}%`,
      ),
      h(
        "div",
        { class: "sysmon-big", style: { color: memColor } },
  ...
  if (variant === "compact") {
    return h(
      "div",
      { class: "sysmon-variant compact" },
      h("div", { class: "sysmon-cpu-model" }, stats.cpu_model.substring(0, 30)),
      h(
        "div",
        { class: "sysmon-bar-group" },
        h(
          "div",
          { class: "sysmon-bar-header" },
          h("span", { class: "sysmon-bar-label" }, "CPU"),
          h(
            "span",
            { class: "sysmon-bar-value", style: { color: cpuColor } },
            `${stats.cpu_usage.toFixed(1)}%`,
          ),
        ),
        h(
          "div",
          { class: "sysmon-bar-track" },
          h("div", {
      ...
      h(
        ...
        h(
          ...
          h(
            "span",
            { class: "sysmon-bar-value", style: { color: memColor } },
        ...
        h(
          ...
          h("div", {
            class: "sysmon-bar-fill",
            style: { width: `${stats.memory_usage}%`, background: memColor },
          }),
        ),
      ),
      h(
        "div",
        { class: "sysmon-load-row" },
  ...
  const swapPct =
    stats.swap_total > 0 ? (stats.swap_used / stats.swap_total) * 100 : 0;
  const swapColor =
    swapPct < 50 ? "#9c27b0" : swapPct < 80 ? "#ff9800" : "#f44336";

  return h(
    "div",
    { class: "sysmon-variant detailed" },
    ...
    stats.swap_total > 0 &&
      h(
        ...
        h(
          ...
          h(
            "span",
            { class: "sysmon-bar-value", style: { color: swapColor } },
            `${swapPct.toFixed(1)}%`,
          ),
        ),
        h(
          ...
          h("div", {
            class: "sysmon-bar-fill",
            style: { width: `${swapPct}%`, background: swapColor },
          }),
        ),
        h(
          "div",
          { class: "sysmon-bar-detail" },
          `${fmtBytes(stats.swap_used)} / ${fmtBytes(stats.swap_total)}`,
        ),
      ),
    h(
      "div",
      { class: "sysmon-info-grid" },
      h(
        "div",
        { class: "sysmon-info-item" },
...
function SendHotkeyWidget({ settings }: { settings: Record<string, any> }) {
  ...
  async function handleExecute() {
    ...
    try {
      ...
      const res = combo
        ? await sendHotkeyCombo(combo)
        : await executeAction("Send Hotkey");
      setResult(res);
    } catch (e) {
      setResult("Error");
    }
  ...
  if (variant === "detailed") {
    return h(
      "div",
      { class: "action-single-widget detailed" },
