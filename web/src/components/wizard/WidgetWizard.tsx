...
import { WizardRequirements } from "./WizardRequirements";
  ...
  return h(
    ...
    h(
      ...
      current > 0
        ? h(
            "button",
            { class: "btn btn-ghost wizard-btn", type: "button", onClick: () => goTo(current - 1) },
      ...
      isLast
        ? h(
            "button",
            { class: "btn btn-success wizard-btn", type: "button", onClick: handleApply },
            t("widget.wizard.save"),
          )
        : h(
            "button",
            {
              class: `btn btn-primary wizard-btn${blocker ? " blocked" : ""}`,
              type: "button",
...
function Overlay({ onClose, children }: { onClose: () => void; children?: any }) {
  return h(Modal, { onClose, sheet: true, class: "wizard-modal" }, children);
}
